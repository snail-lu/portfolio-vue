import{S as x,C as b,P as z,W as C,V as v,B as S,F as u,a as y,A as I,b as A,c as B}from"./threejs.66e2067b.js";import{O as T}from"./OrbitControls.c70c0f2e.js";import{_ as W}from"./index.92839dfe.js";import{z as k,o as E,m as F}from"./elementPlus.307743df.js";const H={id:"star-river"},R={__name:"home",setup(L){const _=()=>{let i=new x;i.background=new b(2632246);let t=new z(60,innerWidth/innerHeight,1,1e3);t.position.set(0,5,49);let o=new C;const g=(Math.floor(innerHeight*devicePixelRatio)/devicePixelRatio).toFixed(2);o.setSize(innerWidth,g),document.querySelector("#star-river").appendChild(o.domElement),window.addEventListener("resize",e=>{t.aspect=innerWidth/innerHeight,t.updateProjectionMatrix(),o.setSize(innerWidth,innerHeight)});let r=new T(t,o.domElement);r.enableDamping=!0,r.enablePan=!1;let l={time:{value:0}},a=[],d=[],c=()=>{d.push(Math.random()*Math.PI,Math.random()*Math.PI*2,(Math.random()*.9+.1)*Math.PI*.1,Math.random()*.9+.1)},m=new Array(5e4).fill().map(e=>(a.push(Math.random()*1.5+.5),c(),new v().randomDirection().multiplyScalar(Math.random()*.5+9.5)));for(let e=0;e<1e5;e++){let p=10,h=40,f=Math.pow(Math.random(),1.5),w=Math.sqrt(h*h*f+(1-f)*p*p);m.push(new v().setFromCylindricalCoords(w,Math.random()*2*Math.PI,(Math.random()-.5)*2)),a.push(Math.random()*1.5+.5),c()}let s=new S().setFromPoints(m);s.setAttribute("sizes",new u(a,1)),s.setAttribute("shift",new u(d,4));let M=new y({size:.125,transparent:!0,depthTest:!1,blending:I,onBeforeCompile:e=>{e.uniforms.time=l.time,e.vertexShader=`
    ...
    `.replace("#include <clipping_planes_fragment>",`#include <clipping_planes_fragment>
        ...
        //if (d > 0.5) discard;
      `).replace("vec4 diffuseColor = vec4( diffuse, opacity );","vec4 diffuseColor = vec4( vColor, smoothstep(0.5, 0.1, d)/* * 0.5 + 0.5*/ );")}}),n=new A(s,M);n.rotation.order="ZYX",n.rotation.z=.2,i.add(n);let P=new B;o.setAnimationLoop(()=>{r.update();let e=P.getElapsedTime()*.5;l.time.value=e*Math.PI,n.rotation.y=e*.05,o.render(i,t)})};return k(()=>{_()}),(i,t)=>(E(),F("div",H))}},$=W(R,[["__scopeId","data-v-e3369bce"]]);export{$ as default};
